...
class Feature(Base):
    ...
    created_at: datetime = Column(DateTime, default=func.now(), nullable=False)
    session_id: Optional[str] = Column(PGUUID(as_uuid=False), ForeignKey("sessions.session_id"), nullable=True)
...
class Session(Base):
    ...
    id: str = Column("session_id", PGUUID(as_uuid=False), primary_key=True, default=lambda: str(uuid.uuid4()))
    ...
    feature = relationship("Feature", foreign_keys=[feature_id], uselist=False)


class Iteration(Base):
    __tablename__ = "iterations"

    id: str = Column("iteration_id", PGUUID(as_uuid=False), primary_key=True, default=lambda: str(uuid.uuid4()))
    session_id: str = Column(PGUUID(as_uuid=False), ForeignKey("sessions.session_id"), nullable=False)
...
class LLMCall(Base):
    ...
    id: str = Column("call_id", PGUUID(as_uuid=False), primary_key=True, default=lambda: str(uuid.uuid4()))
    session_id: str = Column(PGUUID(as_uuid=False), ForeignKey("sessions.session_id"), nullable=False)
